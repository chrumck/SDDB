...
$(document).ready(function () {
    ...
    $("#BtnCreate").click(function () {
        IsCreate = true;
        FillFormForCreate("EditForm", MagicSuggests, "Create Component Type", "MainView");
    });
    ...
    $("#ChBoxShowDeleted").change(function (event) {
        ...
        RefreshTable(TableMain, "/ComponentTypeSrv/Get", (($("#ChBoxShowDeleted").prop("checked")) ? false : true));
    });

    //TableMain Component Types
    TableMain = $("#TableMain").DataTable({
        ajax: { url: "/ComponentTypeSrv/Get?getActive=" + (($("#ChBoxShowDeleted").prop("checked")) ? false : true) },
        columns: [
            { data: "Id", name: "Id" },//0
...
function FillFormForEdit() {
    ...
    $.ajax({
        type: "POST", url: "/ComponentTypeSrv/GetByIds", timeout: 20000,
        ...
        .done(function (data) {
            ...
            CurrRecord.CompTypeName = data[0].CompTypeName;
            CurrRecord.CompTypeAltName = data[0].CompTypeAltName;
            ...
            $.each(data, function (i, dbEntry) {
                if (FormInput.CompTypeName != dbEntry.CompTypeName) FormInput.CompTypeName = "_VARIES_";
                if (FormInput.CompTypeAltName != dbEntry.CompTypeAltName) FormInput.CompTypeAltName = "_VARIES_";
            ...
            $("#EditFormLabel").text("Edit Component Type");

            $("#CompTypeName").val(FormInput.CompTypeName);
            $("#CompTypeAltName").val(FormInput.CompTypeAltName);
...
function SubmitEdits() {
    ...
    $.each(ids, function (i, id) {
        ...
        editRecord.CompTypeName = ($("#CompTypeName").data("ismodified")) ? $("#CompTypeName").val() : CurrRecord.CompTypeName;
        editRecord.CompTypeAltName = ($("#CompTypeAltName").data("ismodified")) ? $("#CompTypeAltName").val() : CurrRecord.CompTypeAltName;
    ...
    $.ajax({
        type: "POST", url: "/ComponentTypeSrv/Edit", timeout: 20000, data: { records: editRecords }, dataType: "json",
        beforeSend: function () { $("#ModalWait").modal({ show: true, backdrop: "static", keyboard: false }); }
    })
        .always(function () { $("#ModalWait").modal("hide"); })
        .done(function (data) {
...
function DeleteRecords() {
    var ids = TableMain.cells(".ui-selected", "Id:name").data().toArray();
    $.ajax({
        type: "POST", url: "/ComponentTypeSrv/Delete", timeout: 20000, data: { ids: ids }, dataType: "json",
        beforeSend: function () { $("#ModalWait").modal({ show: true, backdrop: "static", keyboard: false }); }
    })
        .always(function () { $("#ModalWait").modal("hide"); })
        .done(function () { RefreshTable(TableMain, "/ComponentTypeSrv/Get", (($("#ChBoxShowDeleted").prop("checked")) ? false : true)); })
        .fail(function (xhr, status, error) { ShowModalAJAXFail(xhr, status, error); });
}
