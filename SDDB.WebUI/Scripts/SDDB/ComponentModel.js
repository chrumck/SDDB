...
$(document).ready(function () {
    ...
    $("#BtnCreate").click(function () {
        IsCreate = true;
        FillFormForCreate("EditForm", MagicSuggests, "Create Component Model");
...
function FillFormForEdit() {
    ...
    $.ajax({
        type: "POST", url: "/ComponentModelSrv/GetByIds", timeout: 20000,
        ...
        .done(function (data) {
            ...
            CurrRecord.CompModelName = data[0].CompModelName;
            CurrRecord.CompModelAltName = data[0].CompModelAltName;
            ...
            $.each(data, function (i, dbEntry) {
                if (FormInput.CompModelName != dbEntry.CompModelName) FormInput.CompModelName = "_VARIES_";
                if (FormInput.CompModelAltName != dbEntry.CompModelAltName) FormInput.CompModelAltName = "_VARIES_";
            ...
            $("#CompModelAltName").val(FormInput.CompModelAltName);
...
function SubmitEdits() {
    ...
    $.each(ids, function (i, id) {
        ...
        editRecord.CompModelName = ($("#CompModelName").data("ismodified")) ? $("#CompModelName").val() : CurrRecord.CompModelName;
        editRecord.CompModelAltName = ($("#CompModelAltName").data("ismodified")) ? $("#CompModelAltName").val() : CurrRecord.CompModelAltName;
    ...
    $.ajax({
        type: "POST", url: "/ComponentModelSrv/Edit", timeout: 20000, data: { records: editRecords }, dataType: "json",
        beforeSend: function () { $("#ModalWait").modal({ show: true, backdrop: "static", keyboard: false }); }
    })
        .always(function () { $("#ModalWait").modal("hide"); })
        .done(function (data) {
...
function DeleteRecords() {
    var ids = TableMain.cells(".ui-selected", "Id:name").data().toArray();
    $.ajax({
        type: "POST", url: "/ComponentModelSrv/Delete", timeout: 20000, data: { ids: ids }, dataType: "json",
        beforeSend: function () { $("#ModalWait").modal({ show: true, backdrop: "static", keyboard: false }); }
    })
        .always(function () { $("#ModalWait").modal("hide"); })
