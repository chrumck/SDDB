    ...
    public class ComponentStatusService
    {
        ...
        public virtual Task<List<ComponentStatus>> GetAsync(bool getActive = true)
        {
            using (var dbContextScope = contextScopeFac.CreateReadOnly())
            {
                ...
                return dbContext.ComponentStatuss.Where(x => x.IsActive == getActive).ToListAsync();
            }
        }
        ...
        public virtual Task<List<ComponentStatus>> GetAsync(string[] ids, bool getActive = true)
        {
            ...
            {
                var dbContext = dbContextScope.DbContexts.Get<EFDbContext>();
                return dbContext.ComponentStatuss.Where(x => x.IsActive == getActive && ids.Contains(x.Id)).ToListAsync();
            }
        }
        ...
        public virtual Task<List<ComponentStatus>> LookupAsync(string query = "", bool getActive = true)
        {
            ...
            {
                var dbContext = dbContextScope.DbContexts.Get<EFDbContext>();
                return dbContext.ComponentStatuss.Where(x => x.IsActive == getActive &&
                ...
                using (var trans = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    foreach (var record in records)
                    {
                        ...
                        else
                        {
                            ...
                            var properties = typeof(ComponentStatus).GetProperties(BindingFlags.Public | BindingFlags.Instance).ToArray();
