    ...
    public class AssemblyTypeService
    {
        ...
        public virtual Task<List<AssemblyType>> GetAsync(bool getActive = true)
        {
            using (var dbContextScope = contextScopeFac.CreateReadOnly())
            {
                ...
                return dbContext.AssemblyTypes.Where(x => x.IsActive == getActive).ToListAsync();
            }
        }
        ...
        public virtual Task<List<AssemblyType>> GetAsync(string[] ids, bool getActive = true)
        {
            ...
            {
                var dbContext = dbContextScope.DbContexts.Get<EFDbContext>();
                return dbContext.AssemblyTypes.Where(x => x.IsActive == getActive && ids.Contains(x.Id)).ToListAsync();
            }
        }


        //lookup by query
        public virtual Task<List<AssemblyType>> LookupAsync(string query = "", bool getActive = true)
        {
            using (var dbContextScope = contextScopeFac.CreateReadOnly())
            {
                var dbContext = dbContextScope.DbContexts.Get<EFDbContext>();
                return dbContext.AssemblyTypes.Where(x => x.IsActive == getActive &&
                ...
                using (var trans = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    foreach (var record in records)
                    {
                        ...
                        else
                        {
                            ...
                            var properties = typeof(AssemblyType).GetProperties(BindingFlags.Public | BindingFlags.Instance).ToArray();
