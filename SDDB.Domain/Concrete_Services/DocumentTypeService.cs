    ...
    public class DocumentTypeService
    {
        ...
        public virtual Task<List<DocumentType>> GetAsync(bool getActive = true)
        {
            using (var dbContextScope = contextScopeFac.CreateReadOnly())
            {
                ...
                return dbContext.DocumentTypes.Where(x => x.IsActive == getActive).ToListAsync();
            }
        }
        ...
        public virtual Task<List<DocumentType>> GetAsync(string[] ids, bool getActive = true)
        {
            ...
            {
                var dbContext = dbContextScope.DbContexts.Get<EFDbContext>();
                return dbContext.DocumentTypes.Where(x => x.IsActive == getActive && ids.Contains(x.Id)).ToListAsync();
            }
        }
        ...
        public virtual Task<List<DocumentType>> LookupAsync(string query = "", bool getActive = true)
        {
            ...
            {
                var dbContext = dbContextScope.DbContexts.Get<EFDbContext>();
                return dbContext.DocumentTypes.Where(x => x.IsActive == getActive &&
                ...
                using (var trans = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    foreach (var record in records)
                    {
                        ...
                        else
                        {
                            ...
                            var properties = typeof(DocumentType).GetProperties(BindingFlags.Public | BindingFlags.Instance).ToArray();
